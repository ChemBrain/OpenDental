...
namespace OpenDentBusiness {
	public class PayConnect {
			...
			PayConnectService.signatureResponse sigResponse,long clinicNum) 
		{
			...
			int sigResponseStatusCode=GetSigResponseStatusCode(sigResponse);
			return BuildReceiptString(request.TransType,response.RefNumber,request.NameOnCard,request.CardNumber,request.MagData,response.AuthCode,
				response.Status.description,response.Messages==null ? null : response.Messages.ToList(),request.Amount,sigResponseStatusCode,clinicNum);
		}
		...
		private static int GetSigResponseStatusCode(PayConnectService.signatureResponse sigResponse) {
			return (sigResponse!=null && sigResponse.Status!=null) ? sigResponse.Status.code : 0;
		}
