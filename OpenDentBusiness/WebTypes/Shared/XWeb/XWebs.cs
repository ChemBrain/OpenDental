...
namespace OpenDentBusiness.WebTypes.Shared.XWeb {
	public class XWebs {
		...
		public static XWebResponse GetOtkStatus(long patNum,string orderId) {
			//No need to check RemotingRole;no call to db.
			return SendEdgeExpressRequest(patNum,EdgeExpressTransactionType.QueryPayment,_edgeExpressDirectPayUrl,orderId:orderId);
		}
		#endregion
	...
	public enum EdgeExpressTransactionType {
		///<summary>Charging a credit card.</summary>
		CreditSale,
		///<summary>Authorized a card for a particular amount. Used for creating a token.</summary>
		CreditAuth,
