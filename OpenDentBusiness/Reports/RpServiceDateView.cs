	...
	public class RpServiceDateView {
		...
		private static string GetCoreQuery(long patNum,bool isFamily,int payPlanVersion) {
			...
			command+=$@"
				...
				FROM claimproc 
				INNER JOIN patient ON patient.PatNum=claimproc.PatNum 
				WHERE IF(claimproc.Status IN ({POut.Int((int)ClaimProcStatus.Received)},{POut.Int((int)ClaimProcStatus.Supplemental)}),claimproc.WriteOff > 0,(claimproc.WriteOff > 0 OR claimproc.InsPayEst > 0)) 
			...
			command+=@"
				...
				'WriteOff','Adj','InsPay','PayPlan Credit','PayPlan Charge','PatPay PayPlan') ";
			return command;
		}
	}
}
