	...
	public class RepeatCharges {
		...
		public static RepeatChargeResult RunRepeatingCharges(DateTime dateRun,bool doComputeAging=true) {
			...
			try {
				...
				Dictionary<long,OrthoProcLink> dictOrthoProcLinks=null;
				Dictionary<long,OrthoCase> dictOrthoCases=null;
				Dictionary<long,OrthoSchedule> dictOrthoSchedules=new Dictionary<long, OrthoSchedule>();
				Dictionary<long,OrthoPlanLink> dictOrthoSchedulePlanLinks=null;
				if(OrthoCases.HasOrthoCasesEnabled()) {
					OrthoCases.GetDataForListPatNums(ref dictOrthoProcLinks,ref dictOrthoCases,ref dictOrthoSchedulePlanLinks,ref dictOrthoSchedules,
						listRepeatingCharges.Select(x => x.PatNum).ToList());
				}
				foreach(RepeatCharge repeatCharge in listRepeatingCharges) {
					...
					foreach(DateTime billingDate in listBillingDates) {
						Procedure procAdded=AddProcForRepeatCharge(repeatCharge,billingDate,dateRun,dictOrthoProcLinks,dictOrthoCases,dictOrthoSchedules
						...
						if(repeatCharge.CreatesClaim && !ProcedureCodes.GetProcCode(repeatCharge.ProcCode).NoBillIns) {
							listClaimsAdded=AddClaimsHelper(repeatCharge,procAdded,dictOrthoProcLinks,dictOrthoCases,dictOrthoSchedules);
						}
		...
		private static List<Claim> AddClaimsHelper(RepeatCharge repeateCharge,Procedure proc,Dictionary<long,OrthoProcLink> dictOrthoProcLinks=null,
			Dictionary<long,OrthoCase> dictOrthoCases=null,Dictionary<long,OrthoSchedule> dictOrthoSchedules=null)
		{
			...
			if(orthoProcLink!=null) {
				...
				listOrthoProcLinksAllForOrthoCase=dictOrthoProcLinks.Values.Where(x => x.OrthoCaseNum==orthoCase.OrthoCaseNum).ToList();
			}
			Procedures.ComputeEstimates(proc,proc.PatNum,new List<ClaimProc>(),true,insPlanList,patPlanList,benefitList,pat.Age,subList,orthoProcLink,
				orthoCase,orthoSchedule,listOrthoProcLinksAllForOrthoCase);
		...
		public static Procedure AddProcForRepeatCharge(RepeatCharge repeatCharge,DateTime billingDate,DateTime dateNow,
			Dictionary<long,OrthoProcLink> dictOrthoProcLinks=null,Dictionary<long,OrthoCase> dictOrthoCases=null,
			Dictionary<long,OrthoSchedule> dictOrthoSchedules=null,Dictionary<long,OrthoPlanLink> dictOrthoSchedulePlanLinks=null)
		{
			//No remoting role check; no call to db
			...
			try {
				...
				OrthoCase activeOrthoCase=dictOrthoCases?.Values.FirstOrDefault(x => x.PatNum==pat.PatNum && x.IsActive==true);
				...
				if(activeOrthoCase!=null) {
					listOrthoProcLinksForCase=dictOrthoProcLinks.Values.Where(x => x.OrthoCaseNum==activeOrthoCase.OrthoCaseNum).ToList();
					willProcLinkToOrthoCase=OrthoProcLinks.WillProcLinkToOrthoCase(pat.PatNum,procCode.ProcCode,ref activeOrthoCase,ref listOrthoProcLinksForCase);
					dictOrthoSchedules.TryGetValue(activeOrthoCase.OrthoCaseNum,out orthoSchedule);
					dictOrthoSchedulePlanLinks.TryGetValue(orthoSchedule.OrthoScheduleNum,out orthoSchedulePlanLink);
				}
				//No recall synch needed because dental offices don't use this feature
				Procedures.Insert(procedure,isRepeatCharge: true,skipDiscountPlanAdjustment: willProcLinkToOrthoCase);
				if(willProcLinkToOrthoCase) {
					Procedure procOld=procedure.Copy();
					orthoProcLink=OrthoProcLinks.LinkProcForActiveOrthoCase(procedure,activeOrthoCase,listOrthoProcLinksForCase,orthoSchedulePlanLink,orthoSchedule);
					Procedures.Update(procedure,procOld);
					if(orthoProcLink!=null && orthoProcLink.ProcLinkType!=OrthoProcType.Banding) {
						dictOrthoProcLinks.Add(orthoProcLink.ProcNum,orthoProcLink);
					}
				}
			}
			catch(Exception e) {
