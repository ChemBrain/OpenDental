	...
	public class ClaimProcs{
			...
			,List<OrthoProcLink> listProcLinksForOrthoCase)
		{
			...
			foreach(ClaimProc claimProcCur in listClaimProcsToUpdate) {
				...
				if(procLink.ProcLinkType==OrthoProcType.Visit && !claimProcCur.InsEstTotalOverride.IsZero()) {
					...
					if(visitCount>=plannedVisitsCount && !sumClaimProcEstimates.IsEqual(feeIns-insEstimateForDebond)) {
