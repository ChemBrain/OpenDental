...
namespace OpenDentBusiness {
	public class Procedures {
			...
			List<Procedure> listProcsForAppt,List<InsSub> subList,Userod curUser)
		{
			...
			OrthoProcLink orthoProcLink;
			bool isProcLinkedToOrthoCase=false;
			if(OrthoCases.HasOrthoCasesEnabled()) {//If ortho cases are turned on, check if the patient has an active ortho case.
				activeOrthoCase=OrthoCases.GetActiveForPat(patient.PatNum);//If patient has an active ortho case, get all other ortho case info.
			...
			foreach(Procedure procCur in listProcsForAppt) {
				...
				orthoProcLink=null;
				if(activeOrthoCase!=null) {
					//This will change ProcCur.ProcFee for object in memory.
					orthoProcLink=OrthoProcLinks.LinkProcForActiveOrthoCase(procCur,activeOrthoCase,listOrthoProcLinksForActiveCase,orthoSchedulePlanLink,orthoSchedule);
				}
