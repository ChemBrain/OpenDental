	...
	public class Appointments {
		...
		public static void ValidateApptForWeb(Appointment appt,bool allowDoubleBooking=false) {
			...
			if(!allowDoubleBooking && Appointments.TryAdjustAppointmentInCurrentOp(appt,false,true,out isPatternChanged,out doesOverlapBlockout)) {
				throw new ODException("Appointment overlaps existing "+(doesOverlapBlockout ? "blockout" : "appointment")
					+". Please change the appointment length.");
			}
		...
		public static string ReplaceAppointment(string message,Appointment apt,bool isHtmlEmail=false) {
			...
			ReplaceTags.ReplaceOneTag(template,"[ApptTimeAskedArrive]",apt.DateTimeAskedToArrive.ToShortTimeString(),isHtmlEmail);
