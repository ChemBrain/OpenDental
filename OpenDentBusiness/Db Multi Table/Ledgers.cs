...
namespace OpenDentBusiness{
	...
	public class Ledgers {
		...
		public static string GetAgingQueryString(DateTime asOfDate,List<long> listGuarantors=null,bool isHistoric=false,bool isInsPayWoCombined=true,
			bool hasDateLastPay=false,bool isGroupByGuar=true,bool isWoAged=false,bool doAgePatPayPlanPayments=false)
		{
			//No need to check RemotingRole; no call to db.
			...
			bool doAgePatPayPlanPayments=false,bool doIncludeProcNum=false,bool isAgedByProc=false)
		{
			...
			if(payPlanVersionCur.In(PayPlanVersions.AgeCreditsAndDebits,PayPlanVersions.AgeCreditsOnly)) {
				...
				command+=$@"
					...
					UNION ALL 
						SELECT adjustment.PatNum,adjustment.AdjAmt Fee,payplanlink.PayPlanLinkNum LinkNum,0 ProcNum,DATE('0001-01-01') AgeDate 
						FROM payplanlink 
