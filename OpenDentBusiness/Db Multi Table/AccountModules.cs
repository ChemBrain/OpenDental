	...
	public class AccountModules {
		...
		public static LoadData GetAll(long patNum,bool viewingInRecall,DateTime fromDate,DateTime toDate,bool intermingled,bool showProcBreakdown,
			...
			Action getPrePayForFam=new Action(() => {
				Logger.LogAction("PaySplits.GetPrepayForFam",LogPath.AccountModule,() => retVal.ListPrePayments=PaySplits.GetPrepayForFam(fam,false,isAccountModule:true));
		...
		public static DataSet GetSuperFamAccount(Statement stmtCur,bool isComputeAging=true,bool doIncludePatLName=false
			,bool doShowHiddenPaySplits=false) {
			...
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetDS(MethodBase.GetCurrentMethod(),stmtCur,isComputeAging,doIncludePatLName,doShowHiddenPaySplits);
			}
		...
		public static DataSet GetAccount(long patNum,Statement stmt,bool isComputeAging=true,bool doIncludePatLName=false
			...
			//This method does not call the database directly but still requires a remoting role check because it calls a method that uses out variables.
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetDS(MethodBase.GetCurrentMethod(),patNum,stmt,isComputeAging,doIncludePatLName,doShowHiddenPaySplits);
			}
			bool showProcBreakdown=false;
			if(!stmt.IsInvoice) {
				showProcBreakdown=PrefC.GetBool(PrefName.StatementShowProcBreakdown);
			...
			,bool isComputeAging=true,bool doIncludePatLName=false,List<long> listPayPlanNumsExclude=null,bool doShowHiddenPaySplits=false) 
		{
			...
			if(!string.IsNullOrWhiteSpace(familyPatNums) && stmt.StatementType!=StmtType.LimitedStatement) {//Should not show in limited statements
				...
				//The only difference between the columns below and the columns above are that we use 'payment.PatNum' and '0 AS splitAmt_'.
				command+="UNION ALL "
					...
					+"WHERE payment.PatNum="+pat.PatNum+" "
					//paysplit must be to someone outside this family
					+"AND paysplit.PatNum NOT IN("+familyPatNums+") "
					...
					//hide certain unearned type paysplits
					+((listHiddenUnearnedDefNums.IsNullOrEmpty() || doShowHiddenPaySplits) ? "" 
						: "AND paysplit.UnearnedType NOT IN ("+string.Join(",",listHiddenUnearnedDefNums)+") ")
			...
			for(int i=0;i<rawPay.Rows.Count;i++){
				...
				if(payamt!=amt){
					long payNum=PIn.Long(rawPay.Rows[i]["PayNum"].ToString());//Payment.PayNum for this row.
					...
					if(payNum.In(listPaysplits.Select(x => x.PayNum))) {
						row["description"]+=" "+Lans.g("ContrAccount","(split)");
					}
			...
			if(payPlanVersionCur==PayPlanVersions.AgeCreditsAndDebits || payPlanVersionCur==PayPlanVersions.AgeCreditsOnly) {
				...
				command=$@"
					SELECT payplanlink.AmountOverride,payplanlink.FKey,payplanlink.LinkType,prodlink.Fee,prodlink.Num,payplan.PatNum
					,payplanlink.SecDateTEntry,prodlink.ClinicNum,payplan.PayPlanNum,prodlink.ProvNum
					FROM payplanlink
					...
					LEFT JOIN (
						SELECT procedurelog.PatNum,procedurelog.ProcFee*(procedurelog.UnitQty+procedurelog.BaseUnits)
							+SUM(COALESCE(adjustment.AdjAmt,0))
							+SUM(COALESCE((CASE WHEN claimproc.Status IN ({POut.Int((int)ClaimProcStatus.Received)},{POut.Int((int)ClaimProcStatus.Supplemental)},
								{POut.Int((int)ClaimProcStatus.CapClaim)}) THEN claimproc.InsPayAmt WHEN claimproc.InsEstTotalOverride!=-1 
						...
						LEFT JOIN adjustment ON adjustment.ProcNum=payplanlink.FKey AND payplanlink.LinkType={POut.Int((int)PayPlanLinkType.Procedure)} 
						LEFT JOIN claimproc ON payplanlink.FKey=claimproc.ProcNum 
						...
						UNION ALL
						SELECT adjustment.PatNum,adjustment.AdjAmt Fee,adjustment.AdjNum Num,payplanlink.LinkType
