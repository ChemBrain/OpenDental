...
//		private System.Windows.Forms.PrintDialog printDialog2;
    ...
    private ArrayList ALPreAuth;
		...
		///<summary>Saves/Loads the state of the "Show" checkboxes so that user input can be maintained when switching to/from saved TreatPlans</summary>
		private CheckBoxState _checkboxesShowState;
		...
		public void InitializeOnStartup() {
			...
			//Configure a CheckBoxState variable to save/load the state of all the checkboxes in the "Show" tab.
			_checkboxesShowState=new CheckBoxState(tabPageShow.Controls.OfType<CheckBox>().ToArray());
      ...
      FillPreAuth();
		...
		///<summary>Sets various CheckBoxes to Checked=true based on the data in listRows such that any columns in gridMain that contain relevant data
		///are shown.</summary>
    ...
    private void FillPreAuth(){
      ...
      for(int i=0;i<ALPreAuth.Count;i++){
		...
		private void TrySendTreatPlan(){
			...
			if(gridPlans.SelectedIndices.Count()==0){//Shouldn't happen, control auto selects when loading.
		...
		private PdfDocument GetTreatPlanPDF(out TreatPlan treatPlan,out bool hasPracticeSig){
			...
			if(gridPlans.SelectedIndices.Count()==1) {
				...
				if(DoPrintUsingSheets()) {
					...
					document=SheetPrinting.CreatePdf(sheet);
				}
		...
		private List<ProcTP> LoadActiveTP(ref TreatPlan treatPlan) {
			...
			OrthoCases.GetDataForListProcs(ref listOrthoProcLinksAll,ref dictOrthoProcLinksForProcList,ref dictOrthoCases,ref dictOrthoSchedules,ProcList);
      ...
      //FormClaimEdit2.IsPreAuth=true;
		...
		///<summary>Provides functionality to Save and Load the CheckBox.Checked state of given CheckBoxes.</summary>
		private class CheckBoxState {
			Dictionary<string,bool> _dictCheckStates=new Dictionary<string, bool>();
			List<CheckBox> _listCheckBoxes;
			public CheckBoxState(params CheckBox[] arrCheckBoxes) {
				_listCheckBoxes=arrCheckBoxes.ToList();
			}

			///<summary>Loads the saved CheckBox.Checked state for the saved CheckBoxes.</summary>
			public void Load() {
				foreach(CheckBox checkBox in _listCheckBoxes) {
					if(_dictCheckStates.TryGetValue(checkBox.Name,out bool isChecked)) {
						checkBox.Checked=isChecked;
					}
				}
			}

			///<summary>Saves the CheckBox.Checked state for the CheckBoxes.</summary>
			public void Save() {
				_dictCheckStates.Clear();
				foreach(CheckBox checkbox in _listCheckBoxes) {
					_dictCheckStates[checkbox.Name]=checkbox.Checked;
				}
			}
		}
	}
