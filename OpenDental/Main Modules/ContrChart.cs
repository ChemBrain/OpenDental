...
namespace OpenDental {
	...
	public class ContrChart : UserControl	{
		...
		private UI.ComboBoxPlus comboPriority;
		...
		private UI.ComboBoxPlus comboPrognosis;
		...
		private ODGridPageNav gridProgPageNav;
		...
		private void InitializeComponent(){
			...
			this.comboPriority = new OpenDental.UI.ComboBoxPlus();
			...
			this.comboPrognosis = new OpenDental.UI.ComboBoxPlus();
			...
			this.listDx.SelectedValueChanged += new System.EventHandler(this.listDx_SelectedValueChanged);
			// 
			// labelDx
			// 
			...
			// 
			this.menuItemUngroupMultiVisit.Index = 12;
		...
		public void LayoutToolBar(){
			...
			if(_sheetLayoutControler!=null && _sheetLayoutControler.ListSheetDefsLayout.Count>0) {
				...
				long selectedLayoutSheetDefNum=_sheetLayoutControler.GetLayoutForUser().SheetDefNum;
		...
		private void RefreshModuleData(long patNum,bool isFullRefresh) {
			...
			try {
				Logger.LogAction("GetAll",LogPath.ChartModule,() => _loadData=ChartModules.GetAll(patNum,checkAudit.Checked,GetChartModuleComponents(),
					doMakeSecLog));
			}
		...
		public void RefreshModuleScreen(bool isClinicRefresh=false){
			...
			if(PatCur==null){
				...
				if(PrefC.GetBool(PrefName.PatientDOBMasked)) {
					...
					ContextMenu menu = gridPtInfo.ContextMenu;
		...
		private void Tool_eRx_Click(bool isShowRefillsAndErrors=false) {
			...
			if(erxOption==ErxOption.Legacy) {
				...
				try {
					...
					OIDExternal oIdExternal=DoseSpot.GetDoseSpotPatID(PatCur.PatNum);
					if(oIdExternal==null) {
			...
			ODThread thread=new ODThread((odThread) => {
				...
				try {
					...
					if(!Erx.IsUserAnEmployee(Security.CurUser)) {
						...
						if(PatientNoteCur.Consent.HasFlag(PatConsentFlags.ShareMedicationHistoryErx)) {
							DoseSpot.SetMedicationHistConsent(PatCur,clinicNum);
						}
...
//todo: compare with: Bridges.ECW.AptNum, no need to generate PDF segment, pdfs only with eCW and this button not available with eCW integration
		...
		public void FillPtInfo(bool doRefreshData = true) {
			...
			col=new GridColumn("",0);//HScrollVisible is false, dynamic col width.
			gridPtInfo.ListGridColumns.Add(col);
			gridPtInfo.ListGridRows.Clear();
			GridCell cell;
			GridRow row;
				...
				switch(fieldCur.InternalName) {
					...
					case "Allergies":
						...
						for(int i = 0;allergyList.Count>i;i++) {
							...
							cell=new GridCell(def.Description);
							cell.Bold=YN.Yes;
					...
					case "Problems":
						...
						for(int i=0;i<DiseaseList.Count;i++) {
							...
							if(DiseaseList[i].DiseaseDefNum!=0) {
								cell=new GridCell(DiseaseDefs.GetName(DiseaseList[i].DiseaseDefNum));
								cell.ColorText=Color.Red;
								cell.Bold=YN.Yes;
								row.Cells.Add(cell);
								...
								row.Cells.Add("");
								cell=new GridCell(DiseaseDefs.GetItem(DiseaseList[i].DiseaseDefNum)?.DiseaseName??Lan.g(this,"INVALID PROBLEM"));
								cell.ColorText=Color.Red;
								cell.Bold=YN.Yes;
								row.Cells.Add(cell);
		...
		private void FillTreatPlans() {
			...
			gridTreatPlans.ListGridColumns.Add(new GridColumn(Lan.g("ChartTPList","Heading"),0));
		...
		private void FillTpProcDisplay() {
			...
			gridTpProcs.ListGridColumns.Add(new GridColumn(Lan.g("TableTP","Description"),0));
			gridTpProcs.ListGridRows.Clear();
			if(PatCur==null || _dictTpNumListTpRows==null || gridTreatPlans.ListGridRows.Count==0) {
		...
		private void FillChartViewsGrid(bool doRefreshViews=true) {
			...
			col=new GridColumn(Lan.g("TableChartViews","View"),0);
			gridChartViews.ListGridColumns.Add(col);
		...
		private void FillCustomerViewsGrid() {
			...
			col=new GridColumn(Lan.g("TableCustomerViews","View"),0);
			gridCustomerViews.ListGridColumns.Add(col);
		...
		private void FillProcButtons(bool doRefreshData=true){
			...
			Def selectedButtonCat=listButtonCats.GetSelected<Def>();//Will not be null if 'Quick Buttons' is selected due to above if statement
			List<long> listProcButtonDefNums=Defs.GetDefsForCategory(DefCat.ProcButtonCats,true).Select(x => x.DefNum).ToList();
			if(!listProcButtonDefNums.Contains(selectedButtonCat.DefNum)) {
		...
		private void gridProg_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(row["ProcNum"].ToString()!="0"){
				...
				if(ProcedureCodes.GetStringProcCode(proc.CodeNum,doThrowIfMissing:false)==ProcedureCodes.GroupProcCode){
					...
					FormProcEdit FormP=new FormProcEdit(proc,PatCur,FamCur,listPatToothInitials:ToothInitialList);
		...
		private void AddProcedure(Procedure ProcCur,List<Fee> listFees) {
			...
			FormProcEdit FormPE=new FormProcEdit(ProcCur,PatCur.Copy(),FamCur,listPatToothInitials:ToothInitialList);
			FormPE.IsNew=true;
			FormPE.ShowDialog();
		...
		private void AddQuick(Procedure ProcCur,List<Fee> listFees) {
			...
			if(Programs.UsingOrion) {//Orion requires a DPC to be set. Force the proc edit window open so they can set it.
				//Get from DB to get updated timestamps for permission checks and to initialize nullable variables, like strings, before filling FormProcEdit.
				ProcCur=Procedures.GetOneProc(ProcCur.ProcNum,true);//This breaks the reference to the original Procedure object in the calling method.
				FormP=new FormProcEdit(ProcCur,PatCur.Copy(),FamCur,listPatToothInitials:ToothInitialList);
				FormP.IsNew=true;
		...
		private bool AddProcHelper(Procedure ProcCur,List<Fee> listFees) {
			...
			if(ProcCur.ProvNum==0) {//no proc default prov set, check for appt prov, then use pri prov
				...
				if(Providers.GetProv(provPri).IsHidden) {
					...
					if(PrefC.HasClinicsEnabled){
						provPri=Providers.GetDefaultProvider(PatCur.ClinicNum).ProvNum;
					}
					else {
			...
			OrthoCase activeOrthoCase=null;//Can get set in methods below
			List<OrthoProcLink> listOrthoProcLinksForCase=null;
			bool willProcLinkToOrthoCase=false;
			OrthoProcLink orthoProcLink=null;
			OrthoSchedule orthoSchedule=null;
			if(ProcCur.ProcStatus==ProcStat.C) {
				willProcLinkToOrthoCase=OrthoProcLinks.WillProcLinkToOrthoCase(PatCur.PatNum,procCodeCur.ProcCode,ref activeOrthoCase
					,ref listOrthoProcLinksForCase);
			}
			ProcCur.ProcNum=Procedures.Insert(ProcCur,skipDiscountPlanAdjustment: willProcLinkToOrthoCase);
			if(willProcLinkToOrthoCase) {
				OrthoPlanLink orthoSchedulePlanLink=OrthoPlanLinks.GetOneForOrthoCaseByType(activeOrthoCase.OrthoCaseNum,OrthoPlanLinkType.OrthoSchedule);
				orthoSchedule=OrthoSchedules.GetOne(orthoSchedulePlanLink.FKey);
				Procedure procOld=ProcCur.Copy();
				orthoProcLink=OrthoProcLinks.LinkProcForActiveOrthoCase(ProcCur,activeOrthoCase,listOrthoProcLinksForCase,orthoSchedule:orthoSchedule);
			...
			Procedures.ComputeEstimates(ProcCur,PatCur.PatNum,ref listClaimProcs,true,PlanList,PatPlanList,BenefitList,
				...
				listFees,orthoProcLink:orthoProcLink,orthoCase:activeOrthoCase,orthoSchedule:orthoSchedule,listOrthoProcLinksForOrthoCase:listOrthoProcLinksForCase);
			...
			if(AutoCodeItems.ShouldPromptForCodeChange(ProcCur,procCodeCur,PatCur,isMandibular,listClaimProcs,out verifyCode)) {
				...
				{
					FormProcEdit FormPE=new FormProcEdit(ProcCur,PatCur,FamCur,listPatToothInitials:ToothInitialList);//ProcCur may be modified in this form due to passing by reference. Intentional.
					FormPE.ShowDialog();
			...
			foreach(DataRow row in listSelectedRows) {
				if(row["ProcNum"].ToString()!="0"){
						...
						|| PIn.Bool(row["IsLocked"].ToString()))//takes care of locked group notes and invalidated (deleted and locked) procs
					{
						...
						try {
							dictOrthoProcLinks.TryGetValue(PIn.Long(row["ProcNum"].ToString()),out orthoProcLink);
		...
		private void gridPtInfo_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(gridPtInfo.ListGridRows[e.Row].Tag!=null && gridPtInfo.ListGridRows[e.Row].Tag.ToString()!="DOB") {
				if(new[] { "tabMedical","tabProblems","tabMedications","tabAllergies","tabTobaccoUse" }.Contains(gridPtInfo.ListGridRows[e.Row].Tag.ToString())) {
					FormMedical FormM=new FormMedical(PatientNoteCur,PatCur,gridPtInfo.ListGridRows[e.Row].Tag.ToString());
					FormM.ShowDialog();
					ModuleSelected(PatCur.PatNum);
					return;
				}
				if(gridPtInfo.ListGridRows[e.Row].Tag.GetType()==typeof(RegistrationKey)) {
					...
					FormR.RegKey=(RegistrationKey)gridPtInfo.ListGridRows[e.Row].Tag;
					FormR.ShowDialog();
					FillPtInfo();
					return;
				}
				...
				if(gridPtInfo.ListGridRows[e.Row].Tag.GetType()==typeof(CustRefEntry)) {
					FormReferenceEntryEdit FormRE=new FormReferenceEntryEdit((CustRefEntry)gridPtInfo.ListGridRows[e.Row].Tag);
					FormRE.ShowDialog();
					FillPtInfo();
					return;
				}
					...
					PatField field=(PatField)gridPtInfo.ListGridRows[e.Row].Tag;
		...
		private void menuItemSetComplete_Click(object sender,EventArgs e) {
			...
			List<DataRow> listSelectedRows=gridProg.SelectedIndices.Where(x => x>-1 && x<gridProg.ListGridRows.Count).Select(x => (DataRow)gridProg.ListGridRows[x].Tag).ToList();
			...
			if(listSelectedRows.Count==1 && listSelectedRows[0]["AptNum"].ToString()!="0") {
				...
				FormShortCodeOptIn.PromptIfNecessary(PatCur,apt.ClinicNum);
				return;
			}
		...
		private void MenuItemSetSelectedProcsStatus(ProcStat newProcStatus) {
			...
			foreach(DataRow row in listSelectedRows) {
				...
				orthoProcLink=OrthoProcLinks.GetByProcNum(procOld.ProcNum);
				if(procOld.IsLocked) {
			...
			List<OrthoProcLink> listOrthoProcLinksForActiveCase=null;
			OrthoPlanLink orthoSchedulePlanLink=null;
			OrthoSchedule orthoSchedule=null;
			bool isProcLinkedToOrthoCase=false;
			//If procedures are being set complete and ortho cases are turned on, check if the patient has an active ortho case.
			if(newProcStatus==ProcStat.C && OrthoCases.HasOrthoCasesEnabled()) {
				activeOrthoCase=OrthoCases.GetActiveForPat(PatCur.PatNum);//If patient has an active ortho case, get all other ortho case info.
				if(activeOrthoCase!=null) {
					listOrthoProcLinksForActiveCase=OrthoProcLinks.GetManyByOrthoCase(activeOrthoCase.OrthoCaseNum);
					orthoSchedulePlanLink=OrthoPlanLinks.GetOneForOrthoCaseByType(activeOrthoCase.OrthoCaseNum,OrthoPlanLinkType.OrthoSchedule);
					orthoSchedule=OrthoSchedules.GetOne(orthoSchedulePlanLink.FKey);
				}
			}
			foreach(DataRow row in listSelectedRows) {
				...
				if(procNew.ProcStatus==ProcStat.C) {//Proc set complete.
					...
					if(activeOrthoCase!=null) {
						//This will change procNew.ProcFee for object in memory.
						orthoProcLink=OrthoProcLinks.LinkProcForActiveOrthoCase(procNew,activeOrthoCase,listOrthoProcLinksForActiveCase,orthoSchedulePlanLink,orthoSchedule);
					}
					#endregion
				}
				...
				isProcLinkedToOrthoCase=orthoProcLink!=null;
				Procedures.FormProcEditUpdate(procNew,procOld,procCode,isProcLinkedToOrthoCase);
				if(isProcLinkedToOrthoCase) {//If proc was linked to an ortho case get proc link and pass ortho case data to ComputeEstimates.
					Procedures.ComputeEstimates(procNew,procNew.PatNum,listClaimProcs,false,PlanList,PatPlanList,BenefitList,PatCur.Age,SubList,
						orthoProcLink,activeOrthoCase,orthoSchedule,listOrthoProcLinksForActiveCase);
				}
				else {
					Procedures.ComputeEstimates(procNew,procNew.PatNum,listClaimProcs,false,PlanList,PatPlanList,BenefitList,PatCur.Age,SubList);
				}
		...
		private void menuItemPrintRouteSlip_Click(object sender,EventArgs e) {
			...
			{
				apt=Appointments.GetOneApt(PIn.Long(((DataRow)gridProg.ListGridRows[gridProg.SelectedIndices[0]].Tag)["AptNum"].ToString()));
      ...
      Pen penBlue=new Pen(Color.FromArgb(97,136,173));
		...
		private void listPriorities_MouseDown(object sender,MouseEventArgs e) {
			...
			selectedTpNumProcNums.AddRange(gridTpProcs.SelectedIndices.Where(x => gridTpProcs.ListGridRows[x].Tag!=null).Select(x => (ProcTP)gridTpProcs.ListGridRows[x].Tag)
			...
			foreach(int selectedIdx in gridTpProcs.SelectedIndices) {
				...
				if(clickedRow>0) {//Not 'no priority'
					tpa.Priority=(listPriorities.Items[clickedRow] as ODBoxItem<Def>).Tag.DefNum;
				}
			}
		...
		private void gridTpProcs_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			FormProcEdit FormPE=new FormProcEdit(procCur,PatCur,FamCur,listPatToothInitials:ToothInitialList);
		...
		public void FunctionKeyPressContrChart(Keys keys) {
			...
			switch(keys) {
				case Keys.F1: 
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>0) {
				...
				case Keys.F2:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>1) {
				...
				case Keys.F3:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>2) {
				...
				case Keys.F4:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>3) {
				...
				case Keys.F5:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>4) {
				...
				case Keys.F6:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>5) {
				...
				case Keys.F7:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>6) {
				...
				case Keys.F8:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>7) {
				...
				case Keys.F9:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>8) {
				...
				case Keys.F10:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>9) {
				...
				case Keys.F11:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>10) {
				...
				case Keys.F12:
					...
					if(IsDistributorKey && gridCustomerViews.ListGridRows.Count>11) {
		...
		private void tabProc_MouseDown(object sender,MouseEventArgs e) {
			...
			_sheetLayoutControler.RefreshGridVerticalSpace(GetSheetLayoutMode(),GetSheetFieldDefControlDict(),_sheetLayoutControler.GetPertinentSheetFieldDefs(GetSheetLayoutMode()));
		}
		...
		private void MenuItemPopupUnmaskDOB(object sender,EventArgs e) {
			...
			int idxRowClick = gridPtInfo.PointToRow(_lastClickedPoint.Y);
			int idxColClick = gridPtInfo.PointToCol(_lastClickedPoint.X);//Make sure the user clicked within the bounds of the grid.
		...
		private void MenuItemUnmaskDOB_Click(object sender,EventArgs e) {
			...
			int rowClick = gridPtInfo.PointToRow(_lastClickedPoint.Y);
			gridPtInfo.BeginUpdate();
