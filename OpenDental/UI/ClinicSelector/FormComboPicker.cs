...
namespace OpenDental.UI{
	...
	public partial class FormComboPicker : Form{
		...
		#endregion Events - Key Press

		#region Events - Mouse
		private void FormComboPicker_MouseDown(object sender, MouseEventArgs e){
			_isMouseDown=true;
			if(e.Location.Y<_heightCombo) {//if combobox is hidden (!_isOriginalLocation) height=0
				Close();
				return;//No need to do anything here because the mouse is hovering over the combobox
			}
			_mouseDownIndex=((e.Location.Y-3-_heightCombo)/_heightLineItem)+_indexTopShowing;
			if(!_isMultiSelect) {
		...
		private void FormComboPicker_MouseMove(object sender,MouseEventArgs e) {
			if(e.Location.Y<_heightCombo) {
				_hoverIndex=-1;
				return;//No need to do anything here because the mouse is hovering over the combobox
			}
			//subtract 3 from the location to take border offset into account
			_hoverIndex=((e.Location.Y-3-_heightCombo)/_heightLineItem)+_indexTopShowing; 
		...
		private void FormComboPicker_Load(object sender, EventArgs e){
			...
			int maxItems=(rectScreenBounds.Height/_heightLineItem);
			...
			if(PointInitialUR.Y+(ListStrings.Count*_heightLineItem)+1+_heightCombo>rectScreenBounds.Height) {
				_heightCombo=0;
				//bump it up
				this.Height=(ListStrings.Count*_heightLineItem)+4+_heightCombo;
				this.Top=rectScreenBounds.Height-this.Height;
				_isOriginalLocation=false;
			}
			else{
		...
		private void FormComboPicker_Paint(object sender, PaintEventArgs e){
			...
			if(_isOriginalLocation){
				...
				rectangleCombo.Height=_heightCombo-1;//The minus one is so it's not touching the edge of the control and hiding the drawing
			...
			rectangleOutline.Y=_heightCombo;
			rectangleOutline.Width=this.Width-1;
			//Take the height of the combo box into account
			rectangleOutline.Height=this.Height-_heightCombo-1;
			...
			for(int i=_indexTopShowing; i<totalToDraw; i++) {
				...
				if(isSelected) { //Draw the selected index with blue background
					g.FillRectangle(brushSelectedBack,new Rectangle(1,((i-_indexTopShowing)*_heightLineItem)+2+_heightCombo,this.Width-3,_heightLineItem));
					g.DrawString(ListStrings[i],this.Font,brushSelectedText,2,((i-_indexTopShowing)*_heightLineItem)+2+_heightCombo);
				}
				else if(_hoverIndex==i) { //Draw the hovered index with a light blue background (only when being moused over)
					g.FillRectangle(brushHover,new Rectangle(1,((i-_indexTopShowing)*_heightLineItem)+2+_heightCombo,this.Width-3,_heightLineItem));
					g.DrawString(ListStrings[i],this.Font,Brushes.Black,2,((i-_indexTopShowing)*_heightLineItem)+2+_heightCombo);
				}
