...
namespace OpenDental.UI{
	...
	public partial class ContrApptPanel:UserControl {
		...
		///<summary>Height of single line.  Completely dependent on PrefName.ApptFontSize	or PrefName.ApptPrintFontSize.  We round this number instead of using float font height because screens are still low enough resolution that horizontal lines would look distractingly different.</summary>
		...
		private Font _font8=new Font(FontFamily.GenericSansSerif,8);
		///<summary>This font is used in all the bubble text.</summary>
		private Font _font9=new Font(FontFamily.GenericSansSerif,9);
		///<summary>The header font for the first row of the bubble.</summary>
		private Font _fontBubbleHeader=new Font(FontFamily.GenericSansSerif,10,FontStyle.Bold);
		...
		///<summary>This user control is how we drag appts around in this control as well as over to pinboard.  In the beginning, it gets added to parent ContrAppt, where it stays and gets reused repeatedly.  We control it from here to hide the complexity and also to handle the drawing.</summary>
		...
		protected override void OnMouseUp(MouseEventArgs e) {
			...
			try {
				...
				{//10 or less pixels into the header
					contrTempAppt.Top=this.Location.Y+(int)_heightProvOpHeaders+1;
				}
		...
		public void PrintPage(object sender,System.Drawing.Printing.PrintPageEventArgs e) {
			...
			_font.Dispose();
			_font=new Font(FontFamily.GenericSansSerif,_sizeFont);//Reset to default.
			//_heightLine=g.MeasureString("X",_font,100,StringFormat.GenericTypographic).Height;
			_heightLine=_font.Height;
		...
		public float SizeFont{
			...
			set{
				...
				_sizeFont=value;
				_font.Dispose();
				_font=new Font(FontFamily.GenericSansSerif,_sizeFont);
				_fontCourier.Dispose();
				_fontCourier=new Font(FontFamily.GenericMonospace,_sizeFont);
		...
		///<summary>Width of the provider bar on the left side of each appt.  Gets set from PrefName.ApptProvbarWidth.</summary>
		[Browsable(false)]
		[DefaultValue(8f)]
		...
		///<summary>Call this whenever control should be redrawn because new data has been fetched.  Set an Invalid Flag first.  The Bubble is handled completely separately from this.</summary>
		...
		private void DrawOpsHeader(Graphics g){
			...
			if(IsWeeklyView) {
				for(int d=0;d<_numOfWeekDaysToDisplay;d++) {//for each day of the week displayed
					...
					float widthText=g.MeasureString(strDay,_font8).Width;
					float xPos=d*_widthWeekDay+_widthWeekDay/2-widthText/2;//centered
					g.DrawString(strDay,_font8,_brushTextBlack,new RectangleF(xPos,2,widthText,_heightProvOpHeaders-2));
				...
				for(int i=0;i<_listOpsVisible.Count;i++) {//ops per page in day view
					...
					SizeF sizeTextMeasured=g.MeasureString(ListOpsVisible[i].OpName,_font8);
					...
					g.DrawString(ListOpsVisible[i].OpName,_font8,_brushTextBlack,new RectangleF(locationText.X,locationText.Y,widthText,heightText));
				...
				if(stackBehavior==ApptViewStackBehavior.Vertical) {
					...
					if(isGraphic) {
						...
						if(listApptViewItems[idxItem].ElementDesc=="MedOrPremed[+]"
							...
							|| listApptViewItems[idxItem].ElementDesc=="RecallPastDue[R]"
							|| listApptViewItems[idxItem].ElementDesc=="Prophy/PerioPastDue[P]") 
						{
	...
	///<summary>This was old way of drawing time line across all the bitmaps.</summary>
		...
		private void BubbleDraw(Point point,int idxAppt) {
			...
			float h=0;
			string s;
			Font font=_fontBubbleHeader;//for first row
			for(int i=0;i<listDisplayFieldsBubble.Count;i++) {
				if(listDisplayFieldsBubble[i].InternalName=="Patient Picture") {
				...
				if(x==8){//This is the second line
					x=110;
					y=14;
					font=_font9;
					if(!showingPatientPicture) { 
		...
		private void ComputeHeightProvOpHeader(){
			...
			for(int i=0;i<_listOpsVisible.Count;i++){
				if(TextRenderer.MeasureText(ListOpsVisible[i].OpName,_font8).Width>_widthOpCol){
	...
	///<summary>If we are building our own control class, we don't need this.  This is just for the rare case when we are whipping up a light control that gets painted from the parent.</summary>
