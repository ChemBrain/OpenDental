...
namespace OpenDental.UI {
	...
	public partial class Button : System.Windows.Forms.Button {
		...
		///<summary>This brush is only used for the button panel because the height of those buttons is 18.</summary>
		private static LinearGradientBrush _brushButtonPanel=new LinearGradientBrush(new Point(0,0),new Point(0,18),_colorMainTop,_colorMainBottom);
		...
		protected override void OnPaint(PaintEventArgs p) {
			...
			float radius=3;//radius is no longer part of theme
			GraphicsPath graphicsPath=GraphicsHelper.GetRoundedPath(recOutline,radius);
			...
			if(this.Image!=null) {
				...
				switch(this.ImageAlign) {
					case ContentAlignment.MiddleLeft:
						pointImage.X=6;
						pointImage.Y=this.ClientRectangle.Height/2-Image.Height/2;
						rectangleText.Width=this.ClientRectangle.Width-this.Image.Width;
						rectangleText.Height=this.ClientRectangle.Height;
						rectangleText.X=this.Image.Width;
						rectangleText.Y=0;
						break;
					case ContentAlignment.MiddleRight:
						rectangleText.Width=this.ClientRectangle.Width-this.Image.Width-8;
						rectangleText.Height=this.ClientRectangle.Height;
						rectangleText.X=0;
						rectangleText.Y=0;
					...
					case ContentAlignment.MiddleCenter:// no text in this alignment
						pointImage.X=(this.ClientRectangle.Width-this.Image.Width)/2;
						pointImage.Y=(this.ClientRectangle.Height-this.Image.Height)/2;
				...
				if(this.Enabled) {
					g.DrawImage(this.Image,pointImage);
				}
				else {
		...
		public static void DrawSimpleButton(Graphics g,Rectangle rectangle,string text,Font font){
			RectangleF recOutline=new RectangleF(rectangle.X,rectangle.Y,rectangle.Width-1.2f,rectangle.Height-1.2f);
			float radius=3;
			GraphicsPath graphicsPath=GraphicsHelper.GetRoundedPath(recOutline,radius);
			g.FillPath(_brushButtonPanel,graphicsPath);
			g.DrawPath(_penBorder,graphicsPath);
			graphicsPath.Dispose();
		...
		#region Properties
		///<summary></summary>
		[Category("Misc"),Description("Allows slight adjustments of image location, usually just one or two pixels.")]
