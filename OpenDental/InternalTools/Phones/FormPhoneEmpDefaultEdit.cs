...
namespace OpenDental{
    ...
    "  Used in MapHQ and reserving conference rooms.";
		...
		private void FillGrid() {
			...
			GridColumn col=new GridColumn(Lan.g("TablePhoneGraph","Date"),70);
			col.TextAlign=HorizontalAlignment.Center;
			gridGraph.ListGridColumns.Add(col);
			col=new GridColumn(Lan.g("TablePhoneGraph","IsGraphed"),60);
			col.TextAlign=HorizontalAlignment.Center;
			gridGraph.ListGridColumns.Add(col);
		...
		private void gridGraph_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			//edit this entry
			if(gridGraph.ListGridRows[e.Row].Tag==null || !(gridGraph.ListGridRows[e.Row].Tag is PhoneGraph)) {
				return;
			}
			PhoneGraph phoneGraph=(PhoneGraph)gridGraph.ListGridRows[e.Row].Tag;
