...
namespace OpenDental {
	public partial class FormReleaseCalculator:ODForm {
		...
		private void butCalculate_Click(object sender,EventArgs e) {
			...
			List<Job> listJobs=_listJobsAll.Where(x => x.Priority.In(listPriorities.GetListSelected<Def>().Select(y => y.DefNum)) 
				&& x.PhaseCur.In(listPhases.GetListSelected<JobPhase>())
				&& x.Category.In(listCategories.GetListSelected<JobCategory>())).ToList();
