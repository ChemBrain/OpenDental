...
namespace OpenDental {
	public partial class FormJobManagerOverview:ODForm {
		...
		private void gridSprints_CellClick(object sender,ODGridClickEventArgs e) {
			userControlSprintManager.LoadSprint((JobSprint)gridSprints.ListGridRows[e.Row].Tag,_listJobsAll);
			FillGridEngineers(userControlSprintManager.JobSprintCur);
		}
		...
		private void FillGridJobs(List<Job> listJobs) {
			...
			gridJobs.ListGridColumns.Add(new GridColumn("Last Updated",90) { TextAlign=HorizontalAlignment.Center,SortingStrategy=GridSortingStrategy.DateParse});
		...
		private List<Job> GetFilteredJobList() {
			...
			return listJobs=listJobs.Where(x => x.Priority.In(listPriorities.GetListSelected<Def>().Select(y => y.DefNum))
					&& x.PhaseCur.In(listPhases.GetListSelected<JobPhase>())
					&& x.Category.In(listCategories.GetListSelected<JobCategory>())).OrderByDescending(x => x.HoursActual).ToList();
		}
		...
		private void gridJobs_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			FormJobManager.OpenNonModalJob((Job)gridJobs.ListGridRows[e.Row].Tag,_listJobsAll);
		}
