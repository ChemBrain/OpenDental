...
namespace OpenDental {
	public partial class FormBackport:ODForm {
		...
		private void comboPath_TextChanged(object sender,EventArgs e) {
			...
			string pathCur=(comboPath.Items[0] as string).SubstringBefore("OPEN DENTAL SUBVERSION");
			string pathAfter="";
			ODException.SwallowAnyException(() => {
				pathAfter=comboPath.Text.SubstringBefore("OPEN DENTAL SUBVERSION");
			});
			if(string.IsNullOrEmpty(pathAfter)) {
				ODException.SwallowAnyException(() => {
					pathAfter=comboPath.Text.SubstringBefore("Shared Projects Subversion");
				});
			}
			if(string.IsNullOrEmpty(pathAfter)) {
				return;
