...
namespace OpenDental.InternalTools.Job_Manager {
	...
	public partial class UserControlQueryEdit:UserControl {
		...
		private void FillGridHistory() {
			...
			gridHistory.ListGridColumns.Add(new GridColumn("Date",140));
			gridHistory.ListGridColumns.Add(new GridColumn("User Change",90) { TextAlign=HorizontalAlignment.Center });
			gridHistory.ListGridColumns.Add(new GridColumn("Expert",90) { TextAlign=HorizontalAlignment.Center });
			gridHistory.ListGridColumns.Add(new GridColumn("Engineer",90) { TextAlign=HorizontalAlignment.Center });
			gridHistory.ListGridColumns.Add(new GridColumn("RTF",35) { TextAlign=HorizontalAlignment.Center });//shows X if this row has a copy of job description text.
			gridHistory.ListGridColumns.Add(new GridColumn("Description",300));
			...
			foreach(JobLog jobLog in _jobCur.ListJobLogs.OrderBy(x=>x.DateTimeEntry)) {
				...
				if(checkShowHistoryText.Checked && gridHistory.ListGridRows.Count%2==1) {
		...
		private void gridHistory_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			if(string.IsNullOrWhiteSpace(gridHistory.ListGridRows[e.Row].Cells[4].Text) //because JobLog.MainRTF is not an empty string when it is blank.
				|| !(gridHistory.ListGridRows[e.Row].Tag is JobLog)) 
			{
				return;
			}
			JobLog jobLog = (JobLog)gridHistory.ListGridRows[e.Row].Tag;
