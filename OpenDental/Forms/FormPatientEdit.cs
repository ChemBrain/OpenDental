	...
	public partial class FormPatientEdit : ODForm {
		...
		private void FormPatientEdit_Load(object sender, System.EventArgs e) {
			...
			if(PrefC.GetBool(PrefName.PatientDOBMasked)) {
				...
				butViewBirthdate.Enabled=(Security.IsAuthorized(Permissions.PatientDOBView,true) && textBirthdate.Text!="");//Disable if no DOB
				butViewBirthdate.Visible=true;
			}
			else {
				textBirthdate.Text=Patients.DOBFormatHelper(PatCur.Birthdate,false);
				//butViewBirthdate is disabled and not visible from designer.
				//Move age back over since butViewBirthdate is not showing
				label20.Location=new Point(label20.Location.X-60,label20.Location.Y);
				textAge.Location=new Point(textAge.Location.X-60,textAge.Location.Y);
			}
		...
		private void butShortCodeOptIn_Click(object sender,EventArgs e) {
			FormShortCodeOptIn formSC=new FormShortCodeOptIn(PatCur);
		...
		private void SetRequiredComboBoxPlus(Label labelCur,ComboBoxPlus comboCur,bool areConditionsMet,int disallowedIdx,string errorMsg) {
			SetRequiredControl(labelCur,comboCur,areConditionsMet,disallowedIdx,new List<int>(),errorMsg);
		}
			...
			string errorMsg) 
		{
			if(areConditionsMet) {
				...
				if((contr is ComboBoxMulti && ((ComboBoxMulti)contr).ListSelectedIndices.Exists(x => disallowedIndices.Exists(y => y==x)))
					...
					|| (contr is ComboBoxPlus && ((ComboBoxPlus)contr).SelectedIndex==disallowedIdx)
		...
		private void textBirthdate_Validated(object sender, System.EventArgs e) {
			if(textBirthdate.Text==_maskedDOBOld) {
				textBirthdate.SetError("");//Clear the error, if user doesn't change the text it won't be saved to DB.
				return;//Don't clear date just because user gave focus.  If we can stop validation all together this check isn't needed.
			}
			CalcAge();
		}
		...
		private void ShowPatientEditEmail() {
			FormPatientEditEmail formPatientEditEmail=new FormPatientEditEmail(textEmail.Text);
		...
		private void butViewBirthdate_Click(object sender,EventArgs e) {
			textBirthdate.Text=Patients.DOBFormatHelper(PatOld.Birthdate,false);
			...
			textBirthdate.SetError("");//Clear error so it doesn't look like the birthdate is still invalid.
			textBirthdate.Focus();//Force the validation to happen again when losing focus in case our stored birthdate is already invalid.
		}
		...
		private void butOK_Click(object sender ,System.EventArgs e) {
			...
			foreach(CommOptOutMode mode in listModesOptOut.Where(x => !_listCommOptOuts.Any(y => y.CommMode==x))) {
				...
				//listOptOutsNew.Add(new CommOptOut {
				//	CommMode=mode,
				//	CommType=CommOptOutType.eThankYou,
				//	PatNum=PatCur.PatNum,
				//});
			}
