...
namespace OpenDental {
	...
	public partial class FormRecallList:ODForm {
		...
		private void FillRecalls() {
			...
			//local function for the progress bar.
			void fillGridAction() {
		...
		private void SendWebSched() {
			...
			for(int i=gridRecalls.SelectedIndices.Length-1;i>=0;i--) {
				...
				if(!rowTag.PatNum.In(listPatNumsInTableRecallCur)) {
					skippedNotInList++;
					gridRecalls.SetSelected(gridRecalls.SelectedIndices[i],false);
					continue;
				}
				...
				if(new[] { rowTag.WebSchedSmsSendStatus,rowTag.WebSchedEmailSendStatus }.Any(x => x==AutoCommStatus.SendNotAttempted)
					&& new[] { rowTag.WebSchedSmsSendStatus,rowTag.WebSchedEmailSendStatus }.All(x => x.In(AutoCommStatus.SendNotAttempted,AutoCommStatus.DoNotSend)))
				{
