...
namespace OpenDental {
	...
	public partial class FormAdjMulti:ODForm {
		...
		private void FillGrid() {
			...
			GridColumn col;
			col=new GridColumn(Lan.g("TableMultiAdjs","Date"),70);
			gridMain.ListGridColumns.Add(col);
			col=new GridColumn(Lan.g("TableMultiAdjs","Provider"),90);
			gridMain.ListGridColumns.Add(col);
			if(PrefC.HasClinicsEnabled) {
				col=new GridColumn(Lan.g("TableMultiAdjs","Clinic"),90);
				gridMain.ListGridColumns.Add(col);		
			}		 	 
			col=new GridColumn(Lan.g("TableMultiAdjs","Type"),120);
			gridMain.ListGridColumns.Add(col);			 
		...
		private class MultiAdjEntry {
			...
			public GridRow ToGridRow() {
				...
				if(Proc!=null && Adj==null) {//A procedure row
					...
					//Adj Type
					cell=new GridCell(GetAdjustmentType(Adj));
					row.Cells.Add(cell);
					//Fee
					cell=new GridCell();
					row.Cells.Add(cell);
					...
					row.Cells.Add(cell);
					//Adj Amt
					cell=new GridCell(Math.Round(Adj.AdjAmt,2).ToString());
					cell.ColorBackG=Color.LightCyan;
					row.Cells.Add(cell);
					//Rem After
					cell=new GridCell(Math.Round(RemAfter,2).ToString());
