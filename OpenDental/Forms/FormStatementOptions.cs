		...
		private bool SaveAsDocument(bool printSheet=false,string pdfFileName="",Sheet sheet=null) {
			...
			if(pdfFileName=="" || printSheet) {
				if(checkSuperStatement.Checked) {
					...
					dataSet=AccountModules.GetSuperFamAccount(StmtCur,doIncludePatLName:checkShowLName.Checked,doShowHiddenPaySplits:StmtCur.IsReceipt);
				}
				else {
					dataSet=AccountModules.GetAccount(StmtCur.PatNum,StmtCur,doIncludePatLName:checkShowLName.Checked,doShowHiddenPaySplits:StmtCur.IsReceipt);
				}
				if(sheet==null) {
