...
namespace OpenDental {
	public partial class FormInsRemain:ODForm {
		...
		private void SetGridCols() { 
			...
			col=new GridColumn(Lan.g(gridRemainTimeUnits.TranslationName,"Carrier"),-1);
			gridRemainTimeUnits.ListGridColumns.Add(col);
			col=new GridColumn(Lan.g(gridRemainTimeUnits.TranslationName,"Subscriber"),-1);
			gridRemainTimeUnits.ListGridColumns.Add(col);
			col=new GridColumn(Lan.g(gridRemainTimeUnits.TranslationName,"Category"),-1);
			gridRemainTimeUnits.ListGridColumns.Add(col);
		...
		private void FillGrid() {
			...
			foreach(Benefit benefit in listPatBenefits) {
				...
				foreach(long insSubNum in dictClaimProcsPerSub.Keys) {
					...
					double amtUsed=GetAmtUsedForCat(listFilterProcs,category);
