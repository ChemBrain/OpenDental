...
namespace OpenDental {
	public partial class FormOrthoCase:ODForm {
		...
		public FormOrthoCase(bool isNew,long patNum,OrthoCase orthoCase=null) {
			InitializeComponent();
			Lan.F(this);
			_isNew=isNew;
			_patNum=patNum;
		...
		private void FormOrthoCase_Load(object sender,EventArgs e) {
			...
			DoShowSecondaryIns(PatPlans.GetPatPlansForPat(_patNum).Count>1);
			if(_isNew) {
		...
		private void FillGridOrthoScheduleColumns() {
			...
			gridOrthoSchedule.ListGridColumns.Add(col);
			col=new GridColumn(Lan.g("TableOrthoSchedule","Date Completed"),0,HorizontalAlignment.Right);
			gridOrthoSchedule.ListGridColumns.Add(col);
		...
		private void SetExistingOrthoCaseControls() {
			...
			if(_orthoCaseCur.FeeInsSecondary.IsGreaterThanZero()) {
		...
		private void ButAddBandingProcedure_Click(object sender,EventArgs e) {
			FormProcBandingSelect formProcBandingSelect=new FormProcBandingSelect(_patNum);
		...
		private void TextVisitCountPlanned_TextChanged(object sender,EventArgs e) {
			...
			if(visitCount<0) {
				...
				SetVisitFieldColors(Color.Black);
				if(PIn.Double(textVisitAmount.Text).IsZero()) {
					textVisitPercent.Text="";
					textVisitAmount.Text="";
					textVisitCountPlanned.ForeColor=Color.Red;
				}
		...
		private void TextVisitPercent_TextChanged(object sender,EventArgs e) {
			...
			if(visitPercent.IsLessThan(0) || visitPercent.IsGreaterThan(allVisitsPercent)) {
		...
		private void TextVisitAmount_TextChanged(object sender,EventArgs e) {
			...
			double allVisitsAmount=PIn.Double(textAllVisitsAmount.Text);
			if(visitAmount.IsLessThan(0) || visitAmount.IsGreaterThan(allVisitsAmount)) {
				textVisitCountPlanned.Text="";
				textVisitPercent.Text="";
			...
			,ValidDouble secondLinkedField=null)
		{
			...
			if(changedFieldNumber.IsGreaterThanOrEqualToZero() && changedFieldNumber<=changedFieldLimit) {
		...
		private void SetAmountAndFeeFieldColors() {
			...
			if(sumAmountFields.IsEqual(PIn.Double(textTotalFee.Text))) {
		...
		private void ButDetachProc_Click(object sender,EventArgs e) {
			...
			if(_bandingProc!=null && ((Procedure)gridOrthoSchedule.SelectedGridRows[0].Tag).ProcNum==_bandingProc.ProcNum) {
				...
				try {
					...
					,OrthoProcLinks.GetManyByOrthoCase(_orthoCaseCur.OrthoCaseNum));
		...
		private bool HasErrors() {
			...
			if(textTotalFee.errorProvider1.GetError(textTotalFee)!="" 
				|| textPrimaryInsuranceFee.errorProvider1.GetError(textPrimaryInsuranceFee)!=""
				|| textSecondaryInsuranceFee.errorProvider1.GetError(textSecondaryInsuranceFee)!=""
				|| textPatientFee.errorProvider1.GetError(textPatientFee)!="" 
				|| textBandingAmount.errorProvider1.GetError(textBandingAmount)!=""
				|| textBandingPercent.errorProvider1.GetError(textBandingPercent)!="" 
				|| textDebondAmount.errorProvider1.GetError(textDebondAmount)!=""
				|| textDebondPercent.errorProvider1.GetError(textDebondPercent)!="" 
				|| textAllVisitsAmount.errorProvider1.GetError(textAllVisitsAmount)!=""
				|| textAllVisitsPercent.errorProvider1.GetError(textAllVisitsPercent)!="" 
				|| textBandingDate.errorProvider1.GetError(textBandingDate)!=""
				|| textExpectedDebondDate.errorProvider1.GetError(textExpectedDebondDate)!="" 
				|| textVisitCountPlanned.errorProvider1.GetError(textVisitCountPlanned)!=""
