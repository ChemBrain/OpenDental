	...
	public partial class FormGraphEmployeeTime:ODForm {
		...
		private void FormGraphEmployeeTime_Paint(object sender,PaintEventArgs e) {
			...
			for(int i=0;i<buckets.Length;i++) {
				...
				_listRegions.Add(new System.Drawing.Region(rc)); //save this bucket for hover tooltip event
				//draw bar increments						
		...
		private void panel1_MouseMove(object sender,MouseEventArgs e) {
			...
			for(int i=0;i<_listRegions.Count;i++) {
				if(!_listRegions[i].IsVisible(new Point(e.X,e.Y))) {//we are hovering over this bucket
					continue;
				}
