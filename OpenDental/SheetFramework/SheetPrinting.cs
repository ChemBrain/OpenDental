				...
				if(printRowCur.IsTitleRow) {
					switch(field.FieldName) {//Draw titles differently for different grids.
						case "StatementMain":
							...
							if(gx==null) {
								g.FillRectangle(Brushes.White,field.XPos-10,printRowCur.YPos-_yPosPrint,odGrid.Width+10,heightGridTitle);
								...
								gx.DrawRectangle(Brushes.White,p(field.XPos-10),p(printRowCur.YPos-_yPosPrint-1),p(odGrid.Width+10),p(heightGridTitle));
								using(Font _font=new Font("Arial",10,FontStyle.Bold)) {
									GraphicsHelper.DrawStringX(gx,patName,
										...
										new RectangleF(field.XPos-10,printRowCur.YPos-_yPosPrint-1,odGrid.Width+10,heightGridTitle),HorizontalAlignment.Left);
					...
					switch(field.FieldName) {
						...
						case "StatementInvoicePayment":
							...
							if(gx==null) {
								...
								using(Font _font = new Font("Arial",9,FontStyle.Bold)) {
									if(PrefC.GetBool(PrefName.InvoicePaymentsGridShowNetProd)) {
										GraphicsHelper.DrawStringX(gx,"Total Payments & WriteOffs: "+totalPayments.ToString("c"),new XFont(_font.FontFamily.ToString(),_font.Size,XFontStyle.Bold),XBrushes.Black,new RectangleF(sheet.Width-field.Width-60,printRowCur.YPos-_yPosPrint+_yAdjCurRow,field.Width,heightGridTitle),HorizontalAlignment.Right);
									}
									else {
										GraphicsHelper.DrawStringX(gx,"Total Payments: "+totalPayments.ToString("c"),new XFont(_font.FontFamily.ToString(),_font.Size,XFontStyle.Bold),XBrushes.Black,new RectangleF(sheet.Width-field.Width-60,printRowCur.YPos-_yPosPrint+_yAdjCurRow,field.Width,heightGridTitle),HorizontalAlignment.Right);
									}
								}
							}
							break;
